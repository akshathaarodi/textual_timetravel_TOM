...
out_folder_name = 'outputs-load-test'
...
for i = 1, opt.runs do
    ...
    --final_perf_utest[i] = evaluate('unit-test')
    ...
    if opt.save ~= '' then
       ...
       --torch.save(opt.modelFilename .. '.model', {final_perf_val = final_perf_val, final_perf_test = final_perf_test, model = model, optstate = optstate, weights = weights})
       --torch.save(model, opt.modelFilename .. '.model')
...
--local entnet = nnq(model.network)
--print("Whole network is")
--print(entnet:descendants())
--print(entnet:descendants():attr{name='E'}) 
--print(model.network)


--print("getting predictions")
--print(getNodesByName("predictions").output)
...
function get_all_active_keys()
 ...
 --To print keys and kkeyind

--[[
print("Keys with keyind are")
for i=1, opt.memslots do
   print(i .. " " .. get_words(data,key_names[i],0))
end

print("Data dictionary is")
print(data.idict)
...
function run_example_and_plot(split, question_num, out_folder_name, timestep)
    ...
    local real_keynames = {}
    --Aout2dim = getNodesByName("Aout2dim").output    
--[[    print("Aout2dim here")
    print(Aout2dim)
    print("P out here before")
    print(getNodesByName("predictions").output)
    print("Test softmax here")
    Aout2dim = Aout2dim:float()
    local soft = getNodesByName("predictions")
    
    print(soft:forward(Aout2dim))
    print("P out here after")
    print(getNodesByName("predictions").output)
    print(pred)
    print("prediction is for "..split..question_num..' is '..get_words(sample_data, pred[question_num],1)[1])
    ]] --   
    ...
    print(qs)
    print(get_words(data,answer[example_num],0))
    print(get_words(data,pred[example_num], 1)[1])]]--
    ...
    -- get all the predictions distribution
    
    --print("getting predictions")
    --print(getNodesByName("predictions").output)
    prediction_distribution = getNodesByName("predictions").output[example_num]
    xtic_label_prob = "set xtics ( \""..qs.."\" 0)"
    --ytic_label_prob = "set ytics ("
    --for i=1, opt.memslots do
        --print(get_words(data, key_names[i],0), prediction_distribution[i])
        --ex_prediction:write(get_words(sample_data, key_names[i],0), '  '..prediction_distribution[i].."\n")
        --ytic_label_prob = ytic_label_prob .."\""..get_words(data, key_names[i],0).."\"".." "..(i-1)..","
    --end
    --ytic_label_prob = ytic_label_prob..")"
    -- interested in only first 6 senetnces
...
--test_timetravel(1, 'valid')
--out_folder_name = 'outputs'
--new_output_module(1,70, out_folder_name)

--[[new_output_module(2, 70, out_folder_name)
new_output_module(3, 4, out_folder_name)
new_output_module(4, 70, out_folder_name)
new_output_module(5, 4, out_folder_name)
new_output_module(6, 4, out_folder_name)
--get_output_equation()
--test_softmax()
]]--
...
--get_dotp_dist_mem('valid', 1, 4, ret, out_folder_name)
--get_cosine_dist_mem('valid', 1, 4, ret, out_folder_name)
--get_outmodule(4, 1)
--get_outmodule(70, 1)

run_example_and_plot('valid',2, out_folder_name)
...
--get_dotp_dist_mem('test', 1, ret, out_folder_name)
--get_cosine_dist_mem('test', 1, ret, out_folder_name)

run_example_and_plot('test',2, out_folder_name)
...
--get_output_from_oldtime('valid', 1)
--save_model()


--print("Keys is")
--print(getNodesByName("keys").output)

--print("memo is")
--print(getNodesByName("memo70").output)
