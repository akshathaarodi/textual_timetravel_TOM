...
class WhereObjectAction(InterrogativeAction):
    def __init__(self, oracle: Oracle, obj: str):
        fill = (obj, oracle.get_object_container(obj))
        super().__init__(
            ["Where is the %s?\t%s\t1" % fill,]
        )


class WhereAgentAction(InterrogativeAction):
    def __init__(self, oracle: Oracle, agent: str):
        loc = "unknown" if oracle.get_location(agent) is None else oracle.get_location(agent)
        fill = (agent, loc)
        super().__init__(
            ["What is the location of %s?\t%s\t1" % fill,]
        )
